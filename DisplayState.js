...
import { View, Image, Keyboard, AsyncStorage, BackHandler, Linking } from "react-native"
...
class DisplayState extends React.Component {
  ...
  CloseIt = () => {
    ...
    const duration = moment.range(moment(data.start), moment(Date.now())).valueOf()
    AsyncStorage.getItem("historic").then(historic => {
      historic = JSON.parse(historic) || []
      let data = this.props.notif.data
      data.end = Date.now()
      historic.push(data)
      ...
      Linking.openURL("sms:+33652793171?body=" + "Déplacement APTOR terminé:\nChez " + data.name + " " + data.nickname + "\nD'une durée totale de " + (duration / 1000 / 60).toFixed(0) + " min")
      this.props.quitNotifDetails()
    })
  }
  ...
  render() {
    ...
    return (
      <View style={styles.root}>
        ...
        {this.line("Durée", (duration / 1000 / 60).toFixed(0) + " min")}
        {!data.end &&
